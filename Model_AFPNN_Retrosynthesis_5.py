...
def train(epochs, Datalist,device,Train_list,Test_list,model,model2,optimizer1,optimizer2,loss_fn,loss_fn_3,Labels_dict, Labels, Labels_Features_GCN, model3,AtomSymble_One_Hot,EP_list):
	...
	for epoch in range(epochs):
		...
		for i_test in Test_list:
			...
			for num in range(len(Top5_label)):
				if Top5_label[num] == 1:
					...
					S = Getsubstructure(Smiles_P, bond, step)
					if epoch % 10 == 0:
						if S in T.keys():
							mol = Chem.MolFromSmiles(Smiles_P)
							rxn = AllChem.ReactionFromSmarts(T[S][0])
							x = rxn.RunReactants([mol])
							res = []
							for i in range(len(x)):
								list1 = []
								for j in range(len(x[i])):
									m = x[i][j]
									list1.append((Chem.MolToSmiles(m)))
								res.append(list1)
							# test_result[Smiles_P + '\t' + 'Top-'+str(num+1)] = res
							test_result[Smiles_P] = res
						else:
							# test_result[Smiles_P + '\t' + 'Top-' + str(num+1)] = [['0']]
							test_result[Smiles_P] = [['0']]
							continue
			count_all += 1
		...
		for p in Result:
			tx.write(p)
		if epoch % 10 == 0:
			txx = open('Test-predict.txt', 'w', encoding='UTF-8')
