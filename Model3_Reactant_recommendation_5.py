...
def _invisible_match(smiles):
    atom = {}
    l = (lambda f: f(f, [8], list1, []))(lambda self, out, data, res: res if not data else self(self, out + [out[-1] + data[0]], data[1:], res + [out[-1]]))
    for i in range(len(l)):
    # for i in range(len(list1)):
        atom[Chem.GetPeriodicTable().GetElementSymbol(l[i])] = i
    # print(atom)
    marker = sum([atom.get(smiles, -1)])
...
# 	if Chem.MolToSmiles(Mol_Bond) in ["Cl","Br","O","I","F"]:
# 		return "Br"
# 	else:
# 		return Chem.MolToSmiles(Mol_Bond)
